//! Tests for serializing and deserializing variable-length [`u32`]s.
...
fn assert_eq(var: u32, bytes: Vec<u8>) {
    let mut buffer = Vec::new();
    Standard.serialize_var_u32(var, &mut buffer).unwrap();
    assert_eq!(buffer, bytes, "Expected {var} to serialize as {bytes:?}!");

    let (de, rem) = Standard.deserialize_var_u32(&bytes).unwrap();
