...
fn main() {
    ...
    serialize(&0_u32, &mut buffer).unwrap();
    assert_eq!(buffer, vec![0, 0, 0, 0]);
    buffer.clear();

    // A variable-length `u32` will have a variable number of bytes.
    serialize(&Variable(0_u32), &mut buffer).unwrap();
    assert_eq!(buffer, vec![0]);
    buffer.clear();

    serialize(&127_u32, &mut buffer).unwrap();
    assert_eq!(buffer, vec![127, 0, 0, 0]);
    buffer.clear();

    serialize(&Variable(127_u32), &mut buffer).unwrap();
    assert_eq!(buffer, vec![127]);
    buffer.clear();

    serialize(&Variable(128_u32), &mut buffer).unwrap();
    assert_eq!(buffer, vec![128, 1]);
    buffer.clear();

    serialize(&Variable(255_u32), &mut buffer).unwrap();
    assert_eq!(buffer, vec![255, 1]);
    buffer.clear();

    serialize(&Variable(256_u32), &mut buffer).unwrap();
    assert_eq!(buffer, vec![128, 2]);
    buffer.clear();

    serialize(&Variable(2_097_151_u32), &mut buffer).unwrap();
    assert_eq!(buffer, vec![255, 255, 127]);
    buffer.clear();

    serialize(&2_147_483_647_u32, &mut buffer).unwrap();
    assert_eq!(buffer, vec![255, 255, 255, 127]);
    buffer.clear();

    serialize(&Variable(2_147_483_647_u32), &mut buffer).unwrap();
    assert_eq!(buffer, vec![255, 255, 255, 255, 7]);
    buffer.clear();

    serialize(&4_294_967_295_u32, &mut buffer).unwrap();
    assert_eq!(buffer, vec![255, 255, 255, 255]);
    buffer.clear();

    serialize(&Variable(4_294_967_295_u32), &mut buffer).unwrap();
    ...
    assert_eq!(buffer, vec![66, 0, 99, 0, 0, 0]);
    buffer.clear();
    ...
    assert_eq!(buffer, vec![4, 123, 0, 0, 0, 234, 0, 0, 0, 55, 2, 0, 0, 122, 3, 0, 0]);
    buffer.clear();
    ...
    assert_eq!(buffer, vec![4, 123, 234, 1, 183, 4, 250, 6]);
    buffer.clear();
}
